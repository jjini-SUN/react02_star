...
{
  //starCount(별개수)는 5개를 기본값으로, 컴포넌트를 사용하는 곳에서 정할 수 있게 매개변수로 
  //state 2개 만들기 (별점, 마우스오버 위치)
  let [score, setScore] = useState(0);
  let [hover, setHover] = useState(0);

  //배열 : 총 별 개수(starCount) --> 반복문 map
  let stars = [];  //그려줄 별의 개수를 배열크기로
  ...
  function handleMouseLeave () {
    ...
    //별에서 마우스 나갔다? 점수랑 동일하게 맞춰준다(색깔 맞추기 위해서)
    setHover(score);
  }
  ...
  return(
    <>
      <div>
        {
          //jsx안에다가 자바스크립트 넣으려면 {}
          //map : 배열의 개수만큼 반복, 첫번재는 각 내부요소의 값, 두번째는 인덱스(위치)
          //map 안에 콜백함수를 넣어서 동작
          stars.map((e, idx)=>{
            // e 랑 idx 다른 용어로 써도 됨
            // e : 1, 2, 3, 4, 5
            ...
            return(
              //반드시 하나로 감싸져 있어야함. div여러개 할거면 전체 감싼거 하나 생성
